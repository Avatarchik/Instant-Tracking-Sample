...
/*
	TODOs:
	- play/pause/stop
	- ensure movie is paused on application pause and resumed afterwards
	- movie texture preview in editor (first video frame, or better: frame from mid of video) possible?
*/
...
public class metaioMovieTexture : MonoBehaviour
{
	...
	/// <summary>
	/// Defines whether movie should be played when Start() is called. Start may be called after play/pause/stop because
	/// Unity only initializes scripts once they are used.
	/// </summary>
	private bool initiallyPlaying = false;
	
	/// <summary>
	/// Loop mode to use if initiallyPlaying=true 
	/// </summary>
	private bool initiallyLoop = false;
	
	// Pointer to the movie geometry
	private IntPtr movieGeometry = IntPtr.Zero;
	...
	private void autoResizeGameObject()
	{
		...
		else
		{
			...
			float movieAspect = metaioSDK.getMovieTextureDisplayAspect(movieGeometry); // NOT movieWidth/movieHeight, this is the DAR
	...
	{
		uint movieWidth = metaioSDK.getMovieTextureWidth(movieGeometry);
		uint movieHeight = metaioSDK.getMovieTextureHeight(movieGeometry);
	...
	void OnDestroy()
	{
		metaioSDK.deleteMovieGeometry(movieGeometry);
	}
	...
	public void pause()
	{
		if (movieGeometry == IntPtr.Zero)
			...
			metaioSDK.pauseMovieTexture(movieGeometry);
	}
	...
	public void play(bool loop)
	{
		if (movieGeometry == IntPtr.Zero)
		{
			...
			metaioSDK.startMovieTexture(movieGeometry, loop ? 1 : 0);
	}
	...
	void Start()
	{
		...
		if (fullPath == null)
		{
			Debug.LogError("Movie texture file not found in streaming assets");
			return;
		}
		
		movieGeometry = metaioSDK.createGeometryFromMovie(fullPath);
		...
		metaioSDK.setMovieTextureTargetTextureID(movieGeometry, textureID);
	...
	public void stop()
	{
		if (movieGeometry == IntPtr.Zero)
			...
			metaioSDK.stopMovieTexture(movieGeometry);
	}
}
